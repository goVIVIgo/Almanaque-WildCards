          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              {acoes && acoes.length > 0 ? (
                acoes.map(acao => (
                  <div key={acao.acaoID} className="mt-2">
                    <h5 className="font-semibold text-gray-800">{acao.custo}</h5>
                    <p className="text-sm text-gray-600">{acao.descricaoAcao}</p>
                  </div>
                ))
              ) : (
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              {atributos && atributos.length > 0 ? (
                atributos.map(attr => (
                  <div key={attr.atributoID} className="mt-2">
                    <h5 className="font-semibold text-gray-800">{attr.nomeAtributo}</h5>
                    <p className="text-sm text-gray-600">{attr.descricaoAtributo}</p>
                  </div>
                ))
              ) : (
